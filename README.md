...
- **数据流**
  ...
  2. 生成开篇（LLM）
  3. 从段落末尾提炼"故事线索"（LLM）
  4. 对线索向量化并在 ChromaDB 检索候选图片
  5. 基于新图片续写故事，并回到步骤 3-4，直至结束
  6. 展示完整图文故事

```mermaid
flowchart LR
    A[用户选择起点/主题] --> B[LLM 生成开篇]
    B --> C[LLM 提取故事线索]
    C --> D[Embedding 向量化]
    D --> E[ChromaDB 相似度检索]
    E --> F[选择下一张图片]
...
### Prompt 设计与关键策略
- **图片深度标注（预处理阶段）**
  - 目标：产出有"故事潜力"的描述，包含场景、角色、情绪、潜在线索（时间、地点、物件）。
  - 示例要点：限制长度；输出 JSON 字段（scene, objects, emotions, hooks）。
- **开篇生成**
  - 输入：起始图片的描述或主题
  - 输出：吸引人的第一段，并在结尾埋下"线索"。
- **线索提炼**
  - 要求：从段落结尾提取 1-3 个名词/短语，便于向量检索。
  - 方案 A：LLM 直接输出关键词。方案 B：先输出一句话总结，再精炼关键词。
- **检索与重排**
